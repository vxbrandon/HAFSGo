...
class COCOB(Optimizer):
    def __init__(self, decay_rate=0.001, alpha=100, use_locking=False, name='COCOB'):
        '''
    ...
    def _create_slots(self, var_list):
        for v in var_list:
            ...
            # self._get_or_make_slot(v, decay, "decay", self._name)
    ...
    def _apply_dense(self, grad, var):
        # decay = self.get_slot(var, "decay")  # decay rate
        ...
        # decay_update = 1 / (1 + self.decay_rate / self.epoch) * decay
        # var_update = var - tilde_w + new_w
        var_update = var - (tilde_w - new_w)
        tilde_w_update=new_w

        # decay_update_op = state_ops.assign(decay, decay_update)
